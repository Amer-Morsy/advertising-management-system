...
export const getUsersDialog = async (params) => {
  ...
  try {
    ...
    users.filterData.inPlaceToken = params.inPlaceToken || "";
    ...
    if (response.data.status == STATUS.SUCCESS) {
      for (let item in itemsData) {
        options.push({
          ...
          email: itemsData[item]["userEmail"],
...
export const getEducationalGroupEmployeesDialog = async (params = {}) => {
...
import apiWorkFields from "./../api/settings/settingsOthers/workFields";

export const getWorkFieldsDialog = async (params = {}) => {
  let workFields = new WorkFields();
  workFields.filterData.activationTypeTokens = ACTIVATION_TYPE_TOKENS;
  let options = [];
  try {
    workFields.filterData.placeTypeToken = params.placeTypeToken;

    const response = await apiWorkFields.getDialog(workFields.filterData);
    const itemsData = response.data.itemsData;
    if (response.data.status == STATUS.SUCCESS) {
      for (let item in itemsData) {
        options.push({
          ...
          image: fullPathFileFromServer(
            itemsData[item]["itemImagePath"],
            workFields.workField.defaultImg
          ),
        });
      }
    } else if (response.data.status == STATUS.INVALID_TOKEN) {
      ...
      options.push({
        value: "",
        text: response.data.msg,
        image: workFields.workField.defaultImg,
      });
    }
  } catch (error) {
    options.push({
...
export const getJobOrderStagesDialog = async (params = {}) => {
  let jobOrderStages = new JobOrderStages();
  jobOrderStages.filterData.activationTypeTokens = ACTIVATION_TYPE_TOKENS;
  let options = [];
  try {
    //#region Filter
    jobOrderStages.filterData.userToken = params.userToken ?? "";
    //#endregion Filter

    const response = await apiJobOrderStage.getDialog(
      jobOrderStages.filterData
    );
    const itemsData = response.data.itemsData;
    if (response.data.status == STATUS.SUCCESS) {
      for (let item in itemsData) {
        options.push({
          ...
          image: fullPathFileFromServer(
            itemsData[item]["itemImagePath"],
            jobOrderStages.jobOrderStage.defaultImg
          ),
        });
      }
    } else if (response.data.status == STATUS.INVALID_TOKEN) {
      ...
      options.push({
        value: "",
        text: response.data.msg,
        image: jobOrderStages.jobOrderStage.defaultImg,
      });
    }
  } catch (error) {
    options.push({
...
export const getReportTemplatesDialog = async (params = {}) => {
  let filterData = {
    filterStatus: true,
    activationTypeTokens: ACTIVATION_TYPE_TOKENS,
    reportTemplateTypeToken: params.reportTemplateTypeToken,
    companyMarkToken: params.companyMarkToken,
  };
  let options = [];
  try {
    ...
    if (response.data.status == STATUS.SUCCESS) {
      for (let item in itemsData) {
        options.push({
          value: itemsData[item]["itemToken"],
          // text: itemsData[item]["itemName"],
          text: itemsData[item]["itemNameAr"],
          image: fullPathFileFromServer(
            itemsData[item]["itemImagePath"],
...
//#region CompanyMarks
import defaultImgCompanyMarks from "@/assets/images/advertisingCompany.svg";
import apiCompanyMarks from "./../api/companyMarks";

export const getCompanyMarksDialog = async () => {
  let filterData = {
    filterStatus: true,
    activationTypeTokens: ACTIVATION_TYPE_TOKENS,
  };
  let options = [];
  try {
    const response = await apiCompanyMarks.getDialog(filterData);
    const itemsData = response.data.itemsData;
    if (response.data.status == STATUS.SUCCESS) {
      for (let item in itemsData) {
        options.push({
...
export const getSuppliersCanWorkInPriceQuotation = async (params = {}) => {
  ...
  try {
    ...
    priceQuotations.filterData.priceQuotationDetailsTokens =
      params.priceQuotationDetailsTokens ?? "";
    priceQuotations.filterData.token = params.token ?? "";

    const response = await apiPriceQuotation.getSuppliers(
      priceQuotations.filterData
    );
    const itemsData = response.data.placesData;
    if (response.data.status == STATUS.SUCCESS) {
      for (let item in itemsData) {
        options.push({
          value: itemsData[item]["placeToken"],
          placeNameCurrent: itemsData[item]["placeNameCurrent"],
          phone: itemsData[item]["placePhone1WithCC"],
          fullCode: itemsData[item]["fullCode"],
          image: fullPathFileFromServer(
            itemsData[item]["placeImagePath"],
...
export const getPriceQuotationsDialogCustomized = async (params = {}) => {
  ...
  try {
    priceQuotations.filterData.paginationStatus = false;
    priceQuotations.filterData.requestPlaceInfoDataInclude = true;
    priceQuotations.filterData.priceQuotationHistoryWorkTypeToken =
      params.priceQuotationHistoryWorkTypeToken ?? "";
    priceQuotations.filterData.requestPlaceToken =
      params.requestPlaceToken ?? "";
    ...
    if (response.data.status == STATUS.SUCCESS) {
      for (let item in itemsData) {
        options.push({
          ...
          }-- ${i18n.t("PriceQuotations.clientName")}: ${
            itemsData[item]["requestPlaceInfoData"]["placeNameEn"]
          }- ${itemsData[item]["requestPlaceInfoData"]["placeNameAr"]}`,
          image: fullPathFileFromServer(
...
export const getStoreItemsDialog = async (params = {}) => {
  ...
  try {
    ...
    storeItems.filterData.systemComponentHierarchyToken =
      params.systemComponentHierarchyToken ?? "";
    storeItems.filterData.getOnlyParentStoreItems =
      params.getOnlyParentStoreItems ?? false;
...
import EmailTemplates from "./../models/emailTemplates/EmailTemplates";
import apiEmailTemplate from "./../api/emailTemplate";

export const getEmailTemplatesDialogCustomized = async (params = {}) => {
  let emailTemplates = new EmailTemplates();
  emailTemplates.filterData.activationTypeTokens = ACTIVATION_TYPE_TOKENS;
  let error = null;
  let options = [];
  try {
    emailTemplates.filterData.userToken = params.userToken ?? "";
    const response = await apiEmailTemplate.getAll(emailTemplates.filterData);
    const itemsData = response.data.emailTemplatesData;
    if (response.data.status == STATUS.SUCCESS) {
      for (let item in itemsData) {
        options.push({
          value: itemsData[item]["emailTemplateToken"],
          // text: itemsData[item]["emailTemplateNameCurrent"],
          text: itemsData[item]["emailTemplateNameAr"],
          body: itemsData[item]["emailTemplateHtmlBody"],
          signature: itemsData[item]["emailTemplateHtmlSignature"],
          image: fullPathFileFromServer(
            itemsData[item]["emailTemplateImagePath"],
            emailTemplates.emailTemplate.defaultImg
          ),
        });
      }
    } else if (response.data.status == STATUS.INVALID_TOKEN) {
...
//#region PoliciesAndTerms

import apiPoliciesAndTerms from "./../api/policiesAndTerms";

export const getPoliciesAndTermsDialog = async () => {
  let options = [];
  try {
    const response = await apiPoliciesAndTerms.getAll({
      activationTypeTokens: ACTIVATION_TYPE_TOKENS,
    });
    const itemsData = response.data.policiesAndTermsData;
    if (response.data.status == STATUS.SUCCESS) {
      for (let item in itemsData) {
        options.push({
          value: itemsData[item]["policiesAndTermToken"],
          text: itemsData[item]["policiesAndTermTitle"],
          body: itemsData[item]["policiesAndTermDescription"],
          image: "",
        });
      }
    } else if (response.data.status == STATUS.INVALID_TOKEN) {
