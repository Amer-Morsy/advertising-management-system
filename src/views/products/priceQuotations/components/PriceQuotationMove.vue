      ...
      if (!this.quotationHistoryWorkTypeToken) {
        ...
        try {
          ...
          switch (this.quotationHistoryWorkTypeToken) {
            case PRICE_QUOTATION_HISTORY_WORK_TYPES.WaitingForSupplier:
              response = await apiPriceQuotation.moveToWaitingForSupplier(
                params
              );
              break;
            case PRICE_QUOTATION_HISTORY_WORK_TYPES.WaitingForCustomerAccept:
