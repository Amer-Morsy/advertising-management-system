<template>
  <b-modal id="JobOrderStagesOfWorkDelete" scrollable centered hide-footer>
    ...
    <div class="modal-footer">
      <button
        ...
        @click.prevent="deleteJobOrderStagesOfWork"
      >
        {{ $t("yes") }}
      </button>
      <button
        ...
        @click.prevent="$bvModal.hide('JobOrderStagesOfWorkDelete')"
      >
        {{ $t("no") }}
      </button>
    </div>
...
export default {
  ...
  methods: {
    async deleteJobOrderStagesOfWork() {
      this.isLoading = true;
      try {
        ...
        const response = await apiJobOrderStagesOfWork.deleteStage(formData);
        if (response.data.status == STATUS.SUCCESS) {
          this.showMsg(response.data.msg, true);
          this.$bvModal.hide("JobOrderStagesOfWorkDelete");
          this.$emit(
            "getDetails",
