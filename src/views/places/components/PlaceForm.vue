      ...
      <div class="my-card">
        <div class="row">
          <CustomSelectBox
            ...
            :className="'col-12'"
            :id="`${id}-workFieldToken`"
            :errors="errors_workFieldToken"
            :value="place.workFieldToken"
            :options="workFieldTokenOptions"
            v-on:changeValue="
              place.workFieldToken = $event;
              $v.place.workFieldToken.$touch();
            "
...
import { getWorkFieldsDialog } from "./../../../utils/dialogsOfApi";
import { checkPrivilege } from "./../../../utils/functions";
import generalMixin from "./../../../utils/generalMixin";
// import Places from "./../../../models/places/Places";
import { getLanguage } from "./../../../utils/functions";
import { SYSTEM_TYPE, PLACE_TYPE_TOKENS } from "./../../../utils/constantLists";
import {
...
export default {
  ...
  watch: {
    ...
    placeTypeToken: function () {
      this.getWorkFieldsDialog();
    },
  },
  props: {
    place: {
  ...
  computed: {
    ...
    // validations
    errors_workFieldToken() {
      let errors = [];
      if (this.$v.place.workFieldToken.$error) {
        if (!this.$v.place.workFieldToken.required)
  ...
  methods: {
    async submitForm() {
      bottomSheetScrollToTop();
      this.$v.$touch();
      if (!this.isPlaceTypeTokenEmployee) {
        if (
          this.$v.place.workFieldToken.required &&
          ((this.$v.place.placeNameAr.required &&
            this.$v.place.placeNameAr.isValidNameAr) ||
            (this.$v.place.placeNameEn.required &&
              this.$v.place.placeNameEn.isValidNameEn)) &&
    ...
    async getWorkFieldsDialog() {
      this.isLoading = true;
      let params = { placeTypeToken: this.placeTypeToken };
      if (this.placeTypeToken)
        this.workFieldTokenOptions = await getWorkFieldsDialog(params);
      else this.workFieldTokenOptions = [];
