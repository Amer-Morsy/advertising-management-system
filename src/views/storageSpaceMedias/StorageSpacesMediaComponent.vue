    ...
    <div class="app-content pagination-with-content">
      <b-tabs v-if="mediaTypesOptions">
        <b-tab
          ...
          :key="index"
          @click="filterByMediaType(option.value)"
          :title-item-class="'custom-tab-outer-li tab-container-media'"
        >
          <template #title>
            <span v-b-popover.hover.bottomright="`${option.text}`">
              ...
              <img
                :src="
                  fullPathFileFromServer(
                    option.image,
                    storageSpaceMedias.storageSpaceMedia.icon
                  )
                "
                :onerror="`this.src='${storageSpaceMedias.storageSpaceMedia.icon}'`"
                class="icon-tab"
              />
              <span class="cut-1line">{{ option.text }}</span>
            </span>
          </template>
        </b-tab>
      </b-tabs>
...
import {
  getDialogOfActivationTypes,
  getDialogOfMediaTypes,
...
export default {
  ...
  data() {
    return {
      mediaTypesOptions: getDialogOfMediaTypes(),
      currentTab: "",
  ...
  async created() {
    ...
    this.currentTab = this.mediaTypesOptions[0].value;
    this.storageSpaceMedias.filterData.storageSpaceMediaFileTypeToken =
      this.currentTab;
    this.getAllStorageSpaceMedias();
  },
