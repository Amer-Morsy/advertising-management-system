...
export default class StoreItem extends BaseEntityModel {
  ...
  fillData(data) {
    if (data) {
      ...
      if (data.placesInfoData != null && data.placesInfoData.length > 0) {
        this.placesInfoData = data.placesInfoData.map((element) => {
          return new PlaceInfoData(element);
        });
        this.placeTokens = data.placesInfoData.map((element) => {
          return element.placeToken;
        });
      }
